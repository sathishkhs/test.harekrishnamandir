...
class Blog extends MY_Controller
{
	...
	public function posts_list()
	{
		...
		foreach ($blog->result() as $row) {
			...
			$author = !empty($this->db->where('id',$row->author)->get('author')->row()->name ) ? $this->db->where('id',$row->author)->get('author')->row()->name : '-';
			$category = !empty($this->db->where('category_id',$row->category_id)->get('blogcategory')->row()->category_name ) ? $this->db->where('category_id',$row->category_id)->get('blogcategory')->row()->category_name : '-';
			$data[] = array(
				$row->id,
				$row->title,
				"https://harekrishnamandir.org/blog/post/".$row->page_slug,
				$author  ,
				$category ,
				$row->created_at,
				$row->updated_at,
